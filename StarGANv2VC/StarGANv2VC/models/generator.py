...
class DResBlk(nn.Module):
    ...
    def _build_weights(self, dim_in, dim_out):
        self.conv1 = nn.Conv2d(dim_in, dim_in, 3, 1, 1)
        self.conv2 = nn.Conv2d(dim_in, dim_out // 2, 3, 1, 1)
        self.dconv2 = nn.Conv2d(dim_in, dim_out // 2, 3, 1, padding=(1, 2), dilation=(1, 2))
        if self.normalize:
            self.norm1 = nn.InstanceNorm2d(dim_in, affine=True)
            self.norm2 = nn.InstanceNorm2d(dim_in, affine=True)
        if self.learned_sc:
    ...
    def _residual(self, x):
        ...
        if self.normalize:
            x = self.norm2(x)
        x = self.actv(x)
        x = torch.cat((self.conv2(x), self.dconv2(x)), 1)
        return x
...
class Generator(nn.Module):
    def __init__(self, dim_in=64, style_dim=64, max_conv_dim=512, w_hpf=0, F0_channel=0):
        super().__init__()
        ...
        for lid in range(repeat_num):
            ...
            self.encode.append(
                DResBlk(dim_in, dim_out, normalize=True, downsample=_downtype))
            self.decode.insert(
        ...
        if F0_channel != 0:
            self.F0_conv = nn.Sequential(
                DResBlk(F0_channel, int(F0_channel / 2), normalize=True, downsample='timepreserve'),
            )
